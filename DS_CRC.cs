...
namespace Developer_Tools
{
    class DS_CRC
    {
		...
		public static byte CRC_BCC_XOR(Byte[] byte_array, int start, int length)
		{
			int result = 0;
			for (int index = 0; index < length; index++)
			{
				result = result ^ byte_array[start + index];
			}
			return ((byte)(result & 0xFF));
		}
		public static byte CRC_MEM(Byte[] byte_array, int start, int length)
		{
			byte result = 0;
			while (length != 0)
			{
				result += byte_array[start + length - 1];
				length--;
			}
			result = (byte)~result;
		...
		public static int get_fcs(byte[] byte_array, int start_loc, int len)
		{
			int i, crc = 0xffff;
			for (i = 0; i < len; i++)
			{
				crc = (crc >> 8) ^ CRC16_TAB[(crc ^ byte_array[start_loc + i]) & 0xFF];
			}
			crc = crc ^ 0xFFFF;
		...
		public static bool check_fcs(byte[] byte_array, int start_loc, int len)
		{
			bool result = false;
			int crc = get_fcs(byte_array, start_loc, len);
			if ((byte_array[start_loc + len] == (crc >> 8)) && (byte_array[start_loc + len + 1] == (crc & 0xFF)))
			{
				result = true;
			}
			return result;
